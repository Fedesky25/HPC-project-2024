...
__device__ __host__ int32_t CanvasAdapter::where(complex_t z) {
    auto row = static_cast<int32_t>(std::round(scale*(z.real() - center.real()))) + (int32_t)(width >> 1);
    auto col = static_cast<int32_t>(std::round(scale*(z.imag() - center.imag()))) + (int32_t)(height >> 1);
